...
// ─── Dashboard “What you’ll see” Descriptions ──────────────────────────────────
const ALGO_GRAPH_TYPE: Record<string, string> = {
  'Merge Sort': 'Animated split-and-merge bar chart',
  'Quick Sort': 'Pivot partition bar animation',
  'Heap Sort': 'Animated binary heap tree',
  'Binary Search': 'Animated shrinking search window',
  'Dijkstra': 'Animated weighted graph with shortest path highlight',
  'Kruskal / Prim': 'MST edge-by-edge construction',
  'Floyd Warshall': 'Animated matrix fill',
  'N-Queens': 'Chessboard placement animation',
  'Huffman Coding': 'Animated frequency tree build',
  '0/1 Knapsack': 'DP table fill animation',
  'Fractional Knapsack': 'Sorted items bar chart',
  'Fibonacci Heap': 'Heap tree animation',
  'Max Flow': 'Flow network animation',
  'Graph Coloring': 'Animated node coloring',
  'Hamiltonian Cycle': 'Path-finding through graph nodes',
  'Branch & Bound': 'Tree pruning animation',
  'NP-Complete': 'Complexity landscape',
  'Approximation': 'Approximation ratio chart',
  'Matrix Chain Multiplication': 'Matrix dimension animation',
  'Multistage Graph': 'Stage graph animation',
};
                ...
                >
                  {unit.algos.map((algo, i) => (
                    <button
                      ...
                      className="w-full flex items-start justify-between gap-4 p-3 rounded-xl transition-all border bg-slate-900/50 hover:bg-slate-800 text-slate-300 border-transparent hover:border-slate-700"
                    >
                      <div className="min-w-0 text-left">
                        <div className="font-semibold text-sm text-slate-200 truncate">{algo}</div>
                        <div className="text-[11px] text-slate-500 leading-snug mt-0.5">
                          {ALGO_GRAPH_TYPE[algo] || 'Interactive visualization & step-by-step animation'}
                        </div>
                      </div>
                      <Activity className="w-4 h-4 opacity-50 mt-0.5 flex-shrink-0" />
                    </button>
                  ))}
                </motion.div>
              )}
