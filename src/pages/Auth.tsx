...
export default function Auth() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setLoading(false);
      if (!isLogin) {
        // Save to real local storage so admin panel sees it
        const existingUsers = JSON.parse(localStorage.getItem('algoverse_users') || '[]');
        const newUser = { id: Date.now(), email, joinedAt: new Date().toISOString(), role: 'Student' };
        localStorage.setItem('algoverse_users', JSON.stringify([...existingUsers, newUser]));
